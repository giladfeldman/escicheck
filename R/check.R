...
compute_and_compare_one <- function(row,
  ...
  # Cycle 3 (T4): set when a standardized regression coefficient (beta) is
  ...
  if (!is.na(effect_reported) && !is.na(canonical_type)) {
    ...
    if (length(same_type_variants) > 0) {
      ...
      # the effect size of a plain t-test. effectcheck's single-predictor
      # standardized_beta_from_t cannot recover a multi-predictor / mediation
  ...
  # An effect size recognised but not recoverable from the reported statistic
  # is neither a verified PASS nor a genuine discrepancy, so the default WARN
  # is misleading - report it as NOTE. Two cases: Stage 1 Gap 3 (Cohen's h on a
  # chi-square) and cycle-3 T4 (a standardized beta on a plain t-test).
  if ((h_chisq_unverifiable || beta_t_unverifiable) && status != "NOTE") {
    status <- "NOTE"
  }
